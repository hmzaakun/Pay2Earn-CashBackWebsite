...
if( !isset($_POST['email']) || empty($_POST['email']) || !isset($_POST['password']) || empty($_POST['password']) ){
	...
	header('location: connexion_entreprise.php?message=Vous devez remplir les 2 champs&type=danger');
	exit;
}
...
if( !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) ){
	...
	header('location: connexion_entreprise.php?message=Email invalide&type=danger');
	exit;
}
...
$q = "SELECT identreprise FROM entreprise WHERE email = :email AND password = :password";
$req = $db->prepare($q);
$req->execute([
	'email' => $_POST['email'],
...
if($user){
	...
	// Redirection vers connexion.php
	header('location: connexion_entreprise.php?message=Identifiants invalides&type=danger');
	exit;
}
