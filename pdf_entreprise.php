...
$req = "SELECT * FROM entreprise WHERE ncarte = ?";
$req = $db->prepare($req);
$req->execute([
    $ncarte
]);
...
$pdf->Cell(0,10,'Ma carte entreprise : ',0,1);
$pdf->Cell(0,10,"Nom :  " . $req['nom'] . "\n" ,0,1);
$pdf->Cell(0,10,'Mail : ' . $req['email'] . "\n",0,1);
$pdf->Cell(0,10,'Numero de telephone : ' . $req['telephone'] . "\n",0,1);
$pdf->Cell(0,10,'Chiffre d\'affaire : ' . $req['ca'] . "\n",0,1);
$pdf->Cell(0,10,'Code de parrainage : ' . $req['codeparrain'] . "\n",0,1);
