...
if(isset($_POST['captcha'])){
    if($_POST['captcha'] == $_SESSION['captcha']){
        ...
        header('location: inscription_entreprise.php?message=Captcha invalide ...');
        exit;
    }
}
...
if( !isset($_POST['email']) || empty($_POST['email']) ||
 !isset($_POST['password']) || empty($_POST['password']) ||
	...
	header('location: inscription_entreprise.php?message=Vous devez  tout remplir.');
...
if ($password==$password2) {
				...
				header('location: inscription_entreprise.php?message=Vos mot de passe ne correspondent pas.&type=danger');
				exit;
			}
...
$q = "SELECT identreprise  FROM entreprise WHERE email = :email";
// Préparation de la db
$req = $db->prepare($q);
// Execution de la requête
$req->execute([
...
if($resultat){
	...
	header('location: inscription_entreprise.php?message=Cet email est déjà utilisé.&type=danger');
	exit;
}
...
$verifos = "SELECT ncarte FROM entreprise WHERE ncarte = ?";
...
$q = "INSERT INTO entreprise (email, password, nom,cle,confirme,telephone,codeparrain,ncarte) VALUES (:email, :password,:nom,:cle , :confirme,:telephone,:codeparrain,:ncarte)";
...
$qaa = "SELECT * FROM entreprise WHERE email = ?";
$reqrr = $db->prepare($qaa);
$reqrr->execute([$email]);
$userinfo = $reqrr->fetch();
$_SESSION['id']=$userinfo['identreprise'];
function smtpmailer($to, $from, $from_name, $subject, $body)
   ...
   //   $mail->AddAttachment($path);
    ...
    $msg = '<a href="https://www.pay2earn.store/verif.php?id='.$_SESSION['id'].'&cle='.$cle.'">Confirmer son compte';
    $error=smtpmailer($to,$from, $name ,$subj, $msg);
    if ($parrainoui==1){


      $insertparrain = $db->prepare("UPDATE entreprise SET parrain = ? WHERE email = ?");
      $insertparrain-> execute(array($parrain,$email));
 ...
 if($reponse){
	...
	header('location: inscription_entreprise.php?message=Erreur lors de la création du compte.&type=danger');
	exit;
}
